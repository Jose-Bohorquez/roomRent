...
import contratoArriendoResolve from './contrato-arriendo-routing-resolve.service';

describe('ContratoArriendo routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IContratoArriendo returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          contratoArriendoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(contratoArriendoResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
