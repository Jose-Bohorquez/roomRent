...
import { sampleWithRequiredData } from '../contrato-arriendo.test-samples';
import { ContratoArriendoService } from '../service/contrato-arriendo.service';

import { ContratoArriendo } from './contrato-arriendo';

vitest.useFakeTimers();

describe('ContratoArriendo Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ContratoArriendo;
  let fixture: ComponentFixture<ContratoArriendo>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ContratoArriendo);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'f993e4a3-6619-4c5a-be51-34708828262f' }], {
    ...
    expect(comp.contratoArriendos()[0]).toEqual(expect.objectContaining({ id: 'f993e4a3-6619-4c5a-be51-34708828262f' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'f993e4a3-6619-4c5a-be51-34708828262f' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.contratoArriendos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'f993e4a3-6619-4c5a-be51-34708828262f' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to contratoArriendoService', () => {
      ...
      vitest.spyOn(service, 'getContratoArriendoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getContratoArriendoIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.contratoArriendosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
