...
import perfilUsuarioResolve from './perfil-usuario-routing-resolve.service';

describe('PerfilUsuario routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPerfilUsuario returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          perfilUsuarioResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(perfilUsuarioResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
