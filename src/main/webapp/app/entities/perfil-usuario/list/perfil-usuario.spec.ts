...
describe('PerfilUsuario Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '3bafd714-d12e-4295-9a25-ef8755df4436' }], {
    ...
    expect(comp.perfilUsuarios()[0]).toEqual(expect.objectContaining({ id: '3bafd714-d12e-4295-9a25-ef8755df4436' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '3bafd714-d12e-4295-9a25-ef8755df4436' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.perfilUsuarios()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '3bafd714-d12e-4295-9a25-ef8755df4436' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to perfilUsuarioService', () => {
      ...
      vitest.spyOn(service, 'getPerfilUsuarioIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.perfilUsuariosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
