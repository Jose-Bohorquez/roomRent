...
import { sampleWithRequiredData } from '../inmueble.test-samples';
...
describe('Inmueble Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '543d8aac-c8b3-49b6-8d46-362c3dce8741' }], {
    ...
    expect(comp.inmuebles()[0]).toEqual(expect.objectContaining({ id: '543d8aac-c8b3-49b6-8d46-362c3dce8741' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '543d8aac-c8b3-49b6-8d46-362c3dce8741' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.inmuebles()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '543d8aac-c8b3-49b6-8d46-362c3dce8741' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to inmuebleService', () => {
      ...
      vitest.spyOn(service, 'getInmuebleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getInmuebleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.inmueblesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
