...
import solicitudArriendoResolve from './solicitud-arriendo-routing-resolve.service';

describe('SolicitudArriendo routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ISolicitudArriendo returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          solicitudArriendoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(solicitudArriendoResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
