...
import { sampleWithRequiredData } from '../solicitud-arriendo.test-samples';

import { SolicitudArriendo } from './solicitud-arriendo';

vitest.useFakeTimers();

describe('SolicitudArriendo Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: SolicitudArriendo;
  let fixture: ComponentFixture<SolicitudArriendo>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(SolicitudArriendo);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '1c33f7d7-8f08-4ba7-b15d-c02a593be236' }], {
    ...
    expect(comp.solicitudArriendos()[0]).toEqual(expect.objectContaining({ id: '1c33f7d7-8f08-4ba7-b15d-c02a593be236' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '1c33f7d7-8f08-4ba7-b15d-c02a593be236' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.solicitudArriendos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '1c33f7d7-8f08-4ba7-b15d-c02a593be236' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to solicitudArriendoService', () => {
      ...
      vitest.spyOn(service, 'getSolicitudArriendoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSolicitudArriendoIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.solicitudArriendosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
