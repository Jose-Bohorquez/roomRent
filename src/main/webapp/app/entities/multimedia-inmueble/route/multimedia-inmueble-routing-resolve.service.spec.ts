...
import multimediaInmuebleResolve from './multimedia-inmueble-routing-resolve.service';

describe('MultimediaInmueble routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IMultimediaInmueble returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          multimediaInmuebleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(multimediaInmuebleResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
