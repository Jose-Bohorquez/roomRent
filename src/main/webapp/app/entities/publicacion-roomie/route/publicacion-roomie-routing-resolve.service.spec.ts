...
import publicacionRoomieResolve from './publicacion-roomie-routing-resolve.service';

describe('PublicacionRoomie routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPublicacionRoomie returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          publicacionRoomieResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(publicacionRoomieResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
