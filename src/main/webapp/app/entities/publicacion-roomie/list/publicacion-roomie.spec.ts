...
import { sampleWithRequiredData } from '../publicacion-roomie.test-samples';
import { PublicacionRoomieService } from '../service/publicacion-roomie.service';

import { PublicacionRoomie } from './publicacion-roomie';

vitest.useFakeTimers();

describe('PublicacionRoomie Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PublicacionRoomie;
  let fixture: ComponentFixture<PublicacionRoomie>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PublicacionRoomie);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '453c2e6e-bf4a-4f0e-be12-a3257c9b7249' }], {
    ...
    expect(comp.publicacionRoomies()[0]).toEqual(expect.objectContaining({ id: '453c2e6e-bf4a-4f0e-be12-a3257c9b7249' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '453c2e6e-bf4a-4f0e-be12-a3257c9b7249' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.publicacionRoomies()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '453c2e6e-bf4a-4f0e-be12-a3257c9b7249' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to publicacionRoomieService', () => {
      ...
      vitest.spyOn(service, 'getPublicacionRoomieIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPublicacionRoomieIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.publicacionRoomiesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
