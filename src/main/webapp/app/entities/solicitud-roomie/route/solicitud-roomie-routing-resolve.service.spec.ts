...
import solicitudRoomieResolve from './solicitud-roomie-routing-resolve.service';

describe('SolicitudRoomie routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ISolicitudRoomie returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          solicitudRoomieResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(solicitudRoomieResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
