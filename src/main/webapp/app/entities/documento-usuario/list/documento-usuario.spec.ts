...
import { sampleWithRequiredData } from '../documento-usuario.test-samples';
...
describe('DocumentoUsuario Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DocumentoUsuario;
  let fixture: ComponentFixture<DocumentoUsuario>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DocumentoUsuario);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'db348ca9-68bd-4486-8a92-f8cef80f77c7' }], {
    ...
    expect(comp.documentoUsuarios()[0]).toEqual(expect.objectContaining({ id: 'db348ca9-68bd-4486-8a92-f8cef80f77c7' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'db348ca9-68bd-4486-8a92-f8cef80f77c7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.documentoUsuarios()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'db348ca9-68bd-4486-8a92-f8cef80f77c7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to documentoUsuarioService', () => {
      const entity = { id: 'db348ca9-68bd-4486-8a92-f8cef80f77c7' };
      vitest.spyOn(service, 'getDocumentoUsuarioIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDocumentoUsuarioIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.documentoUsuariosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
