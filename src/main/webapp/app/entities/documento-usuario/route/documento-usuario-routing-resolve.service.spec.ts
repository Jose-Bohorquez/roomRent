...
import documentoUsuarioResolve from './documento-usuario-routing-resolve.service';

describe('DocumentoUsuario routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IDocumentoUsuario returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          documentoUsuarioResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(documentoUsuarioResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
