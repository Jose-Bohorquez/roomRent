...
import visitaProgramadaResolve from './visita-programada-routing-resolve.service';

describe('VisitaProgramada routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IVisitaProgramada returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          visitaProgramadaResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(visitaProgramadaResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
