...
import { sampleWithRequiredData } from '../publicacion-inmueble.test-samples';
import { PublicacionInmuebleService } from '../service/publicacion-inmueble.service';

import { PublicacionInmueble } from './publicacion-inmueble';

vitest.useFakeTimers();

describe('PublicacionInmueble Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PublicacionInmueble;
  let fixture: ComponentFixture<PublicacionInmueble>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PublicacionInmueble);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '39817e9d-1a0a-463a-b680-3dedcd5663fd' }], {
    ...
    expect(comp.publicacionInmuebles()[0]).toEqual(expect.objectContaining({ id: '39817e9d-1a0a-463a-b680-3dedcd5663fd' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '39817e9d-1a0a-463a-b680-3dedcd5663fd' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.publicacionInmuebles()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '39817e9d-1a0a-463a-b680-3dedcd5663fd' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to publicacionInmuebleService', () => {
      ...
      vitest.spyOn(service, 'getPublicacionInmuebleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPublicacionInmuebleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.publicacionInmueblesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
