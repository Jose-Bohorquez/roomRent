...
import publicacionInmuebleResolve from './publicacion-inmueble-routing-resolve.service';

describe('PublicacionInmueble routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPublicacionInmueble returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          publicacionInmuebleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(publicacionInmuebleResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
